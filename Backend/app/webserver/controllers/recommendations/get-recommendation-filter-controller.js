...
async function getPlaceOrCategoryRecommendations(req, res) {
  ...
  else filter = `%${filter}%`; //todo lo que tenga una parte del filtro
  ...
  try {
    ...
    const [queryCountTotal] = await connection.execute(
      `SELECT COUNT(*) AS totalRecommendations FROM recommendations WHERE place LIKE ? OR category LIKE ?`,
      [filter, filter]
    );
    ...
    if (!orderByLikes) {
      responseBody = {
        ...
        goToFirstPage:
          page > 1
            ? `http://${req.headers.host}/api/recommendations?filter=${
                req.query.filter
              }&page=${1}`
            : null,
        ...
        goToLastPage:
          page < lastPage
            ? `http://${req.headers.host}/api/recommendations?filter=${req.query.filter}&page=${lastPage}`
            : null,
      ...
      responseBody = {
        ...
        goToFirstPage:
          page > 1
            ? `http://${req.headers.host}/api/recommendations?filter=${
                req.query.filter
              }&orderByLikes=${req.query.orderByLikes}&page=${1}`
            : null,
        prev:
          page > 1
            ? `http://${req.headers.host}/api/recommendations?filter=${
                req.query.filter
              }&orderByLikes=${req.query.orderByLikes}&page=${+page - 1}`
            : null,
        ...
        goToLastPage:
          page < lastPage
            ? `http://${req.headers.host}/api/recommendations?filter=${req.query.filter}&orderByLikes=${req.query.orderByLikes}&page=${lastPage}`
            : null,
        recommendations: result[0],
      };
    }
