  ...
  try {
    ...
    if (orderByLikes === "date") {
      result = await connection.execute(
        `SELECT id, photo, title, category, place, intro, content, created_at, user_id FROM recommendations WHERE place LIKE ? OR category LIKE ? ORDER BY created_at DESC, id LIMIT ${maxRecommendationsPerPage} OFFSET ${offset}`,
        [filter, filter]
      );
    }
